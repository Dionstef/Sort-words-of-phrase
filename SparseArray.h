...
public:
  ...
  void insert(T* pObj);                                             // adds element following the last element in the array
...
template<typename T>
void SparseArray<T>::insert(T* pObj)
{
    if (pLast)
  {
    pLast->pPrevious = std::make_shared<Node>(pObj);
    pLast->pPrevious->pNext = pLast;
    pLast = pLast->pPrevious;
  }
  else
    pLast = pFirst = std::make_shared<Node>(pObj);
